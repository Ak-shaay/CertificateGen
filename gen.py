...
def generate_certificate(private_key, subject_details, ca_cert, oid_dict=None, cert_type="end_entity"):
    ...
    # Extract the issuer details from the CA certificate
    ...
    # subject_key_identifier_ca = ca_cert.extensions.get_extension_for_oid(ExtensionOID.SUBJECT_KEY_IDENTIFIER).value
    # print("Subject Key Identifier from CA certificate:", subject_key_identifier)

    # Authority Key Identifier (corrected)
    cert_builder = cert_builder.add_extension(
        AuthorityKeyIdentifier.from_issuer_subject_key_identifier(
    ...
    crl_url = "http://example.com/crl.pem"
...
# Function to load CA certificate and private key with password handling
def load_ca_cert_and_key():
    # Assuming you already have the files
    # Load CA certificate
    with open("ca.crt", "rb") as ca_cert_file:
        ca_cert = load_pem_x509_certificate(ca_cert_file.read(), backend=default_backend())
...
# Function to sign the certificate with the CA's private key
def sign_certificate_with_ca(cert_builder, ca_private_key, ca_cert):
...
def main():
    ...
    # Load CA certificate and private key from the same folder
    ca_cert, ca_private_key = load_ca_cert_and_key()
    ...
    cert_builder = generate_certificate(private_key, subject_details, ca_cert, oid_dict, cert_type)

    # Sign the certificate using the CA's private key
    signed_certificate = sign_certificate_with_ca(cert_builder, ca_private_key, ca_cert)

    # Save the signed certificate and private key
    save_certificate_to_pem(signed_certificate, "signed_cert_"+cert_type+".cer")
